...
\begin{tikzpicture}[scale=0.85]
  ...
  \node[font=\small\bfseries\color{primaryblue}] at (-0.6, 2.45) [rotate=90] {Build};
  \node[font=\small\bfseries\color{primaryblue}] at (7.6,  2.45) [rotate=270] {Analyze};
...
\begin{center}
  {\large\hi{\$245,000}}\quad US avg total comp for AI engineers.\\[0.2em]
...
\begin{columns}[b]
  \begin{column}{0.48\textwidth}
    \begin{center}
      ...
      \begin{tikzpicture}
        \begin{axis}[
          ybar, bar width=22pt, bar shift=0pt,
          clip=false,
          ...
          \node[below, font=\scriptsize, anchor=north] at (axis cs:AI, 0) {AI};
        \end{axis}
      \end{tikzpicture}\\[0.2em]
      {\footnotesize\positive{+36\% premium}}
    \end{center}
  \end{column}
  \begin{column}{0.48\textwidth}
    \begin{center}
      ...
      \begin{tikzpicture}
        \begin{axis}[
          ...
          \addplot[fill=primarygold!50, draw=primarygold] coordinates {(Non-AI,85)};
...
\begin{columns}[T]
  \begin{column}{0.52\textwidth}
    \begin{center}
      ...
      {\footnotesize\neutral{On pace to surpass all of 2024 (\$6.1B full year).}}\\[0.5em]
      \begin{tikzpicture}
        \begin{axis}[
          xbar, bar width=18pt,
          clip=false,
          ...
          y tick label style={font=\scriptsize, anchor=east},
          axis lines=left,
          ...
          \addplot[fill=primarygold!50, draw=primarygold] coordinates {(6.1,2024)};
          \addplot[fill=primaryblue, draw=primaryblue] coordinates {(6.0,{H1 2025})};
          \node[left, font=\scriptsize, anchor=east] at (axis cs:0,{H1 2025}) {H1 2025};
        \end{axis}
      \end{tikzpicture}
    \end{center}
  \end{column}
...
\begin{columns}[T]
  \begin{column}{0.32\textwidth}
    \begin{methodbox}[Navigation]
      \centering
      \includegraphics[width=0.90\linewidth]{../Figures/SUNY/canvas_teaser.png}\\[0.3em]
      {\small\hi{Navigation}}\\[0.2em]
  ...
  \begin{column}{0.32\textwidth}
    \begin{methodbox}[Manipulation]
      \centering
      \includegraphics[width=0.90\linewidth]{../Figures/SUNY/d2e_teaser.png}\\[0.3em]
      {\small\hi{Manipulation}}\\[0.2em]
...
\vspace{0.2em}
\begin{center}
  \includegraphics[width=0.85\textwidth, height=0.52\textheight, keepaspectratio]{../Figures/SUNY/canvas_teaser.png}
\end{center}
\vspace{0.3em}
\begin{itemize}\setlength\itemsep{0.15em}
  \item \hi{InternVL} backbone: \hi{2$\times$ faster} inference $\cdot$ data pipeline \negative{4--5 days} $\rightarrow$ \positive{2--3 hours}
...
\begin{axis}[
  ...
  % Annotation for the dip — raised to avoid collision with value label
  \node[font=\scriptsize, text=negativered, align=center] at (axis cs:2nd eval, 95) {eval set\\changed};
  % Explicit x-axis labels (pgfplots grouped bar layout clips auto-labels)
  \node[font=\scriptsize, anchor=north east, rotate=30] at (axis cs:Sim only, -3) {Sim only};
  \node[font=\scriptsize, anchor=north east, rotate=30] at (axis cs:+Sim2Real, -3) {+Sim2Real};
  \node[font=\scriptsize, anchor=north east, rotate=30] at (axis cs:+Real 2.5h, -3) {+Real 2.5h};
  \node[font=\scriptsize, anchor=north east, rotate=30] at (axis cs:1st eval, -3) {1st eval};
  \node[font=\scriptsize, anchor=north east, rotate=30] at (axis cs:2nd eval, -3) {2nd eval};
  \node[font=\scriptsize, anchor=north east, rotate=30] at (axis cs:3rd eval, -3) {3rd eval};
  \node[font=\scriptsize, anchor=north east, rotate=30] at (axis cs:Final, -3) {Final};
\end{axis}
\end{tikzpicture}
\end{center}
\vspace{0.2em}
...
\begin{columns}[T]
  \begin{column}{0.55\textwidth}
    \begin{center}
      \includegraphics[width=\linewidth, height=0.48\textheight, keepaspectratio]{../Figures/SUNY/d2e_teaser.png}
    \end{center}
  \end{column}
  \begin{column}{0.42\textwidth}
    \begin{center}
      \includegraphics[width=\linewidth, height=0.48\textheight, keepaspectratio]{../Figures/SUNY/d2e_libero.png}
    \end{center}
    \vspace{0.1em}
    {\footnotesize\neutral{LIBERO benchmark results}}
  \end{column}
\end{columns}
\vspace{0.1em}
\begin{itemize}\setlength\itemsep{0.05em}
  \item \hi{1,300h} game data in \hi{under 1 week} $\cdot$ manipulation \positive{96.6\%} on LIBERO, beats \hi{7$\times$ larger} models
...
\begin{columns}[T]
  \begin{column}{0.46\textwidth}
    \begin{enumerate}\setlength\itemsep{0.3em}
      \item \hi{Sim research} — 29\% $\rightarrow$ 50\%
      \item \hi{Lab prototype} — 87.5\% (2.5h data)
      \item \hi{Field evals} — 4 evals in 8 days
      \item \positive{97.3\% success}, 0\% failure
      \item \positive{100+ contracts}, Korea + Japan
    \end{enumerate}
    \vspace{0.6em}
    \begin{keybox}[]
      \footnotesize\itshape ``Research that doesn't ship\\is just a hobby.''
    \end{keybox}
  \end{column}
  \begin{column}{0.52\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../Figures/SUNY/canvas_deployment.png}
  \end{column}
\end{columns}
\end{frame}
...
\begin{keybox}[]
  \centering
  \small But knowing the tools is table stakes. The real career edge in 2026? \textbf{Orchestrating} them.
\end{keybox}
\end{frame}
...
\begin{tabular}{p{4.0cm} p{4.5cm}}
  ...
  Portfolio = coursework    & \positive{Portfolio = initiative} \\[3pt]
  Uses AI tools individually & \positive{Orchestrates multi-agent workflows} \\
  \bottomrule
\end{tabular}
\end{center}
\vspace{0.6em}
\begin{keybox}[]
  \centering
  \small\hi{AI writes the code.\ \textbf{You orchestrate the system.}\ You own the understanding.}
\end{keybox}
\end{frame}

% ---- 4B+: Agent Orchestration (New Slides 45-47) ----

% --- Slide 45: From AI User to AI Architect ---
\begin{frame}{From AI User to AI Architect}
\vspace{0.3em}
\begin{description}[\textbf{Level 3}]
  \item[\hi{Level 1} --- AI User:] Use ChatGPT/Claude for individual tasks \hfill {\neutral{(where most students start)}}
  \item[\hi{Level 2} --- AI Builder:] Build with Cursor/Copilot, deploy AI-powered features \hfill {\neutral{(Slide 40 projects)}}
  \item[\hi{Level 3} --- AI Architect:] Design multi-agent systems, orchestrate AI workflows \hfill {\neutral{(the new frontier)}}
\end{description}
\vspace{0.4em}
\begin{highlightbox}[]
  \small
  Workers in AI-exposed roles earn a \textbf{56\% wage premium}\\
  Skills in AI-exposed roles evolve \textbf{66\% faster}\\[0.2em]
  {\footnotesize\neutral{PwC Global AI Jobs Barometer, 2025}}
\end{highlightbox}
\vspace{0.3em}
\begin{keybox}[]
  \centering
  \small Level 3 is where the hiring premium lives --- and where the fewest candidates are.
\end{keybox}
\end{frame}

% --- Slide 46: What AI Agent Orchestration Actually Looks Like ---
\begin{frame}{What AI Agent Orchestration Actually Looks Like}
\vspace{0.2em}
{\small Multi-agent = multiple AI models collaborating on one task}
\vspace{0.4em}
\begin{center}
\footnotesize
\begin{tabular}{p{4.2cm} p{5cm}}
  \toprule
  \textbf{\neutral{2024: AI Tool User}} & \textbf{\hi{2026: AI System Architect}} \\
  \midrule
  One prompt $\to$ one answer       & \positive{Pipeline: plan $\to$ code $\to$ review $\to$ deploy} \\[3pt]
  Copy-paste between tools          & \positive{Agents hand off work automatically} \\[3pt]
  You manage the workflow           & \positive{The system manages itself} \\
  \bottomrule
\end{tabular}
\end{center}
\vspace{0.3em}
\begin{methodbox}[Who's Hiring for This]
  \footnotesize
  OpenAI --- Agent Infrastructure Engineers\\
  Google --- 40\% of enterprise apps will use AI agents by 2026 {\footnotesize\neutral{Gartner, 2025}}\\
  Anthropic --- Claude Code multi-agent teams\\
  GenAI job postings grew \textbf{170\%} in one year {\footnotesize\neutral{Indeed Hiring Lab, 2025}}
\end{methodbox}
\end{frame}

% --- Slide 47: Build an Agent Portfolio Project ---
\begin{frame}{Build an Agent Portfolio Project}
\vspace{0.2em}
{\small These are Level 3 projects --- beyond Slide 40. They show you can \textbf{orchestrate}, not just use.}
\vspace{0.4em}
\begin{enumerate}
  \item \textbf{Multi-Agent Code Reviewer} --- 3 AI agents (style, bugs, security) review code together. Tools: Claude Code + scripting. \textit{Weekend project.}
  \item \textbf{Research Assistant Pipeline} --- Chain: search papers $\to$ summarize $\to$ compare $\to$ synthesize. Tools: Claude API + LangGraph. \textit{2-week project.}
  \item \textbf{AI Debate Team} --- Agents with different expertise argue a topic, then reach consensus. Tools: CrewAI or Claude teams. \textit{2-week project.}
\end{enumerate}
\vspace{0.3em}
\begin{keybox}[]
  \centering
  \small The students who can orchestrate AI agents will be the ones companies fight over in 2027.
\end{keybox}
\end{frame}
...
\begin{tikzpicture}[scale=1.0]
  ...
  \node[font=\scriptsize\bfseries,  text=positivegreen!70!black, above] at (0, 3.5) {Behavioral};
...
\begin{center}
\begin{tabular}{@{}ll@{}}
  \hi{\$245K} avg AI engineer salary (US) & \higold{50\% of positions unfilled} \\[1.2em]
...
\begin{tabular}{@{}lp{8cm}@{}}
  ...
  {\large\bfseries\higold{AI Tools \& Orchestration}} & Claude, Cursor, agent workflows --- the new career edge. \\[1.2em]
...
\begin{columns}[T]
  \begin{column}{0.42\textwidth}
    \vspace{0.5em}
    \begin{center}
      \qrcode[height=4cm]{https://github.com/alohays}\\[0.6em]
