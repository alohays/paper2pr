...
<!-- Every class from here on is styled by the SLIDE TYPES section of the
theme; the placeholder assets under `assets/` are committed so this deck
renders anywhere. Section breaks are `## {.divider}` slides, never a level-1
heading: a `# ` heading makes reveal nest every following slide under it as a
vertical stack, which is why the quality gate blocks on one. -->
