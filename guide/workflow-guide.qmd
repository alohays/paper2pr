...
| `/new-deck` | New deck of any genre | Starting a paper review, talk, or lecture |
...
- `/new-deck` remains available as a guided creation workflow
...
## Pattern 3: Creating a New Deck {#pattern-3-creating-a-new-lecture}

`/new-deck` is the single entry point for every genre --- paper review, invited talk, or course lecture. It starts by asking what the deck is for, because the quality gate cannot enforce a budget nobody declared:

```
/new-deck
  |
  +-- Phase 1: Interview --- genre, audience, duration, series position,
  |            language, delivery, publication
  +-- Phase 2: Scaffold --- <deck>.deck.yml + qmd stub + Figures dir
  |            (dry-run the config first; it is what the deck is graded on)
  +-- Phase 3: Author in batches of 5-10 slides, against the profile's
  |            budget rather than a remembered one
  +-- Phase 4: Figures (R scripts, /extract-tikz)
...
Decks authored before genres existed were created with `/create-lecture`, which assumed one audience and one length for everything and wrote to a flat `Quarto/`. It was retired once the repo started carrying three genres; `/new-deck` covers what it did.
...
| `/new-deck` | `.claude/skills/new-deck/` | Interview, scaffold, and author a deck |
